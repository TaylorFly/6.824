...
def main(
    ...
    topics = set(topics)
    # Force terminal mode to ensure output works with file redirection
    console = Console(force_terminal=True)
    width = console.size.width or 80  # Fallback width for non-TTY
    ...
    for line in input_:
        try:
            ...
            # Check if this looks like a valid log line (time should be numeric, topic should be known)
            if not time.isdigit() or topic not in TOPICS:
                # This is not a log line, treat as test output
                raise ValueError("Not a log line")
            
            # To ignore some topics (user-specified filter)
            if topic not in topics:
                continue
